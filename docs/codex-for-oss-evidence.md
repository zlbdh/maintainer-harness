...
| External feedback candidate review | `scripts/checks/find-external-feedback-candidates.ps1` scans issue `#5`, `#6`, and `#7` for non-owner, non-bot public comments that are not already registered, then prints guarded pending evidence commands for maintainer review. It does not create comments, stars, or verified evidence by itself. |
