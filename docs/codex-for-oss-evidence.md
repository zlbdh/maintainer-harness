...
| First-run troubleshooting | `docs/first-run-troubleshooting-zh.md` gives real reviewers a short Chinese path through Git, PowerShell, execution policy, working-directory, clipboard, and comment-target issues before they post issue `#6` first-run feedback. |
