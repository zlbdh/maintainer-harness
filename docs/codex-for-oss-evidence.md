...
| 90% scorecard | `docs/codex-for-oss-90-scorecard.md`, `scripts/checks/measure-application-readiness.ps1`, `scripts/checks/assert-form-submission-ready.ps1`, `scripts/checks/test-form-submission-ready.ps1`, and the readiness JSON fixtures under `tests/fixtures/readiness/` define, enforce, and regression-test the hard external-signal gates before asking the maintainer to submit the form. |
...
- The pre-submit form gate is regression-tested with explicit ready and not-ready fixtures, so a 60/90 local state cannot silently become a form-submission recommendation.
