...
| External feedback candidate review | `scripts/checks/find-external-feedback-candidates.ps1` scans issue `#5`, `#6`, and `#7` for non-owner, non-bot public comments that are not already registered, then prints guarded pending evidence commands for maintainer review. The scheduled readiness monitor runs the same scan with `GITHUB_TOKEN` and uploads the candidate JSON as an artifact. It does not create comments, stars, or verified evidence by itself. |
