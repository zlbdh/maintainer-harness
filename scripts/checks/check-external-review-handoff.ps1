...
$fileRequirements = @(
    [pscustomobject]@{
        ...
        Text = @(
            ...
            '10 分钟跑 demo #6',
            '复制邀请消息',
            'data-copy-target="template-invite-zh"',
            'id="template-invite-zh"',
            '不是让你直接 star',
            '你觉得有价值再 star',
            '复制 #5 模板',
            'data-copy-target="template-reviewability-zh"',
            'id="template-reviewability-zh"',
            '复制 #6 模板',
            'data-copy-target="template-first-run-zh"',
            'id="template-first-run-zh"',
...
try {
    ...
    if (([string]$zhPacket.selected_request_text).Contains('不是让你直接 star') -and ([string]$zhPacket.selected_request_text).Contains($links.ChineseFriendOnepager) -and ([string]$zhPacket.selected_request_text).Contains($links.ChineseSendChecklist) -and ([string]$zhPacket.selected_request_text).Contains($links.ChineseFeedbackRecovery) -and ([string]$zhPacket.selected_request_text).Contains($links.RecommendationCheck) -and ([string]$zhPacket.selected_request_text).Contains('-CommentLanguage zh -CopyCommentToClipboard -OpenCommentTarget')) {
